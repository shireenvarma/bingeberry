...
async function purgeExpiredCompletedTasks() {
  try {
    const { error } = await supabase.rpc('purge_expired_completed_tasks');
    // Older databases will not have the migration yet. Do not block sign-in in that case.
    if (error && error.code !== '42883') console.warn('Completed-task cleanup failed:', error.message);
  } catch (error) {
    console.warn('Completed-task cleanup failed:', error.message);
  }
}
...
function quickAddOptions(items, selectedValue = '', emptyLabel = '') {
  const empty = emptyLabel ? `<option value="">${emptyLabel}</option>` : '';
  return empty + items.map((item) => `<option value="${item.id}"${item.id === selectedValue ? ' selected' : ''}>${item.name}</option>`).join('');
}

function quickAddTaskRow() {
  if (!canCreateTask()) return '';
  const clients = isAdmin() || CURRENT_USER?.role === 'manager' ? DB.clients : visibleClients();
  const defaultClientId = activeFilters.client !== 'All' ? activeFilters.client : clients[0]?.id || '';
  const selectedClient = getClient(defaultClientId);
  const typeOptions = clientTypeOptions();
  const assignees = CURRENT_USER?.role === 'team' ? [CURRENT_USER] : DB.users;
  const defaultAssigneeId = CURRENT_USER?.role === 'team' ? CURRENT_USER.id : assignees[0]?.id || '';
  return `<tr class="quick-add-row">
    <td><span class="quick-add-mark">+</span></td>
    <td><input id="qa-name" class="form-control quick-add-input" placeholder="Task name" aria-label="Task name"><input id="qa-brief" class="form-control quick-add-input quick-add-brief" placeholder="Brief (optional)" aria-label="Task brief"></td>
    <td><select id="qa-client" class="form-control quick-add-input" aria-label="Brand">${quickAddOptions(clients, defaultClientId, 'No brand')}</select></td>
    <td><select id="qa-type" class="form-control quick-add-input" aria-label="Type">${typeOptions.map((type) => `<option value="${type}"${type === (selectedClient?.type || 'Retainer') ? ' selected' : ''}>${type}</option>`).join('')}</select></td>
    <td><select id="qa-assigned" class="form-control quick-add-input" aria-label="Assigned"${CURRENT_USER?.role === 'team' ? ' disabled' : ''}>${quickAddOptions(assignees, defaultAssigneeId)}</select></td>
    <td><input id="qa-due" class="form-control quick-add-input" type="date" value="${todayStr}" aria-label="Due date"></td>
    <td><select id="qa-priority" class="form-control quick-add-input" aria-label="Priority"><option>High</option><option selected>Medium</option><option>Low</option></select></td>
    <td><select id="qa-status" class="form-control quick-add-input" aria-label="Status"><option value="todo" selected>To Do</option><option value="progress">In Progress</option><option value="review">In Review</option><option value="blocked">Blocked</option><option value="done">Done</option></select></td>
    <td><button class="btn btn-primary btn-sm quick-add-save" type="button" onclick="saveQuickTask()">Add</button></td>
  </tr>`;
}
...
function fillTable(bodyId, tasks, cols = 'full', options = {}) {
  ...
  const quickAdd = options.quickAdd ? quickAddTaskRow() : '';
  const rows = tasks.length ? tasks.map((task) => taskRow(task, cols, true, options)).join('') : `<tr class="empty-row"><td colspan="${colspan}">No tasks here</td></tr>`;
  body.innerHTML = quickAdd + rows;
}
...
function renderTasksPage() {
  ...
  fillTable('main-task-body', tasks, 'full', { viewKey: 'tasks', lockDoneOrder: true, quickAdd: true });
...
async function saveQuickTask() {
  ...
  const name = byId('qa-name')?.value.trim();
  const due = byId('qa-due')?.value;
  if (!name || !due) {
    showToast('Task name and due date are required.', 'error');
    return;
  }

  const clientId = byId('qa-client')?.value || '';
  const assignedId = CURRENT_USER?.role === 'team' ? CURRENT_USER.id : byId('qa-assigned')?.value || '';
  const status = byId('qa-status')?.value || 'todo';
  const task = {
    id: crypto.randomUUID(),
    name,
    brief: byId('qa-brief')?.value.trim() || '',
    client: clientId,
    clientIds: clientId ? [clientId] : [],
    type: byId('qa-type')?.value || 'Retainer',
    start: '',
    due,
    priority: byId('qa-priority')?.value || 'Medium',
    slot: 'any',
    status,
    assigned: assignedId,
  ...
  try {
    await ensureSession();
    const { error } = await supabase.from('tasks').insert(mapTaskToRow(task));
    if (error) throw error;
    await loadWorkspace();
    renderAll();
...
async function updateTaskField(task, patch, successMessage = '') {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'status')) {
      nextTask.completedAt = patch.status === 'done' ? task.completedAt || new Date().toISOString() : '';
    }
    touchTask(nextTask);
    await ensureSession();
